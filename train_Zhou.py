...
from Models.model_Zhou import build_model
...
def train_intersub(testdata_name, rootpath, feature_type="time"):
    ...
    if feature_type == "time":
        ...
        train_data = train_data.reshape(-1, N_CHANNELS, LEN_SEQ, 1)
        test_data = test_data.reshape(-1, N_CHANNELS, LEN_SEQ, 1)
    elif feature_type == "freq":
        # build model
        model = build_model(n_channels=N_CHANNELS, len_seq=LEN_SEQ//2)
        train_data = np.abs(np.fft.fft(train_data))[:, :, :LEN_SEQ//2]
        test_data = np.abs(np.fft.fft(test_data))[:, :, :LEN_SEQ//2]
        train_data = train_data.reshape(-1, N_CHANNELS, LEN_SEQ//2, 1)
        test_data = test_data.reshape(-1, N_CHANNELS, LEN_SEQ//2, 1)
    else:
        raise ValueError("feature_type is \"time\" or \"freq\"")
...
def train_crossval(rootpath, exp_id=0, feature_type="time"):
    ...
    for cv, (train_idx, test_idx) in enumerate(kf.split(all_data, all_label)):
        ...
        if feature_type == "time":
            ...
            # build model
            model = build_model(n_channels=N_CHANNELS, len_seq=LEN_SEQ//2)
            all_data = np.abs(np.fft.fft(all_data))[:, :, :LEN_SEQ//2]
            all_data = all_data.reshape(-1, N_CHANNELS, LEN_SEQ//2, 1)
        else:
            raise ValueError("feature_type is \"time\" or \"freq\"")
...
if __name__ == '__main__':
    ...
    train_crossval(rootpath, exp_id=exp_id, feature_type="freq")
    """